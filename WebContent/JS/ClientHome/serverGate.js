function ServerGate(view){
    ...
    this.requestClient = function(cback){
        this.makeCall("GET", "getClientInfo", cback);
    }

    this.requestQuotations = function(cback){
        this.makeCall("GET", "getQuotations", cback);
    }

    this.requestAllProducts = function(cback){
        this.makeCall("GET", "getAllProducts", cback);
    }

    this.submitQuotation = function(){
        var path = "CreateQuotation?productID=" + this.model.getProductSelected();

        for (let j = 0; j < this.model.getOptionsSelected().length; j++) {
            const optionID = this.model.getOptionsSelected()[j];
            if(optionID != null){
                path += "&option" + optionID + "=" + optionID ;
            }
        }
        this.makeCall("GET", path, this.orchestrator.goToHome);
    }
